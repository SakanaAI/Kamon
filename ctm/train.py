...
def parse_args():
    ...
    # "You'll probably need a bigger network (e.g., 2048 neurons)"
    ...
    # Per Luke: you'll want to set your '--n_synch_out' to about 1/4 or 1/2 of
    # your total output size. For the maze task there's a length of 100, and
    # this is set to 32 by default. So I'm guessing we should make this about
    # 1/2 of 11, which is the max length of a description in the Kamon dataset.
    parser.add_argument('--n_synch_out', type=int, default=6, help='Number of neurons to use for output synch (CTM only).') # Default changed
    ...
    # "Also, set the '--neuron_select_type' to random pairing, always."
    ...
    # "Note also how the kamon task has a 'cirriculum_lookahead' parameter. You might want to set that to 1."
    parser.add_argument('--cirriculum_lookahead', type=int, default=1, help='How far to look ahead for cirriculum.')


    # Training
    parser.add_argument('--expand_range', action=argparse.BooleanOptionalAction, default=True, help='Kamons between 0 and 1 = False. Between -1 and 1 = True. Legacy checkpoints use 0 and 1.')
...
if __name__=='__main__':
    ...
    train_accuracies_most_certain_perkamon = [] # Full kamon accuracy
    test_accuracies_most_certain_perkamon = []
    iters = []
        ...
        if os.path.isfile(checkpoint_path):
            ...
            if not args.reload_model_only:
                ...
                if not args.ignore_metrics_when_reloading:
                    ...
                    train_accuracies_most_certain_perkamon = checkpoint['train_accuracies_most_certain_perkamon']
                    test_accuracies_most_certain_perkamon = checkpoint['test_accuracies_most_certain_perkamon']
                else:
    ...
    with tqdm(total=args.training_iterations, initial=start_iter, leave=False, position=0, dynamic_ncols=True) as pbar:
        for bi in range(start_iter, args.training_iterations):
            ...
            with torch.autocast(device_type="cuda" if "cuda" in device else "cpu", dtype=torch.float16, enabled=args.use_amp):
                ...
                if args.model == 'ctm':
                    # CTM output: (B, SeqLength*vocab_size, Ticks), Certainties: (B, Ticks)
                    ...
                    # LSTM output: (B, SeqLength*vocab_size, Ticks), Certainties: (B, Ticks)
                    predictions_raw, certainties, synchronisation = model(inputs)
                     # Reshape predictions: (B, SeqLength, vocab_size, Ticks)
                    predictions = predictions_raw.reshape(predictions_raw.size(0), -1, vocab_size, predictions_raw.size(-1))
                    loss, where_most_certain, upto_where = kamon_loss(predictions, certainties, targets, cirriculum_lookahead=args.cirriculum_lookahead, use_most_certain=False)
                    ...
                    # FF has no certainties, pass None. kamon_loss must handle this.
                    # Unsqueeze predictions for compatibility with kamon loss calcluation
                    loss, where_most_certain, upto_where = kamon_loss(predictions.unsqueeze(-1), None, targets, cirriculum_lookahead=args.cirriculum_lookahead, use_most_certain=False)
            ...
            pbar.set_description(f'Dataset=Kamon. Model={args.model}. {pbar_desc}')
            ...
            if bi%args.track_every==0 and (bi != 0 or args.reload_model_only):
                ...
                with torch.inference_mode(): # Use inference mode for tracking
                    ...
                    current_train_accuracies_most_certain_perkamon_eval = []
                    current_test_accuracies_most_certain_perkamon_eval = []
                    ...
                    with tqdm(total=len(loader), initial=0, leave=False, position=1, dynamic_ncols=True) as pbar_inner:
                        for inferi, (inputs, targets) in enumerate(loader):
                            ...
                            if args.model == 'ctm':
                                ...
                                predictions = predictions_raw.reshape(predictions_raw.size(0), -1, vocab_size, predictions_raw.size(-1)) # B,S,C,T
                                loss, where_most_certain, _ = kamon_loss(predictions, certainties, targets, use_most_certain=True)
                                ...
                                predictions_raw, certainties, _ = model(inputs)
                                predictions = predictions_raw.reshape(predictions_raw.size(0), -1, vocab_size, predictions_raw.size(-1)) # B,S,C,T
                                loss, where_most_certain, _ = kamon_loss(predictions, certainties, targets, use_most_certain=False) # where = -1
                                ...
                                predictions = predictions_raw.reshape(predictions_raw.size(0), -1, vocab_size) # B,S,C
                                loss, where_most_certain, _ = kamon_loss(predictions.unsqueeze(-1), None, targets, use_most_certain=False) # where = -1
                    ...
                    # Calculate full kamon accuracy at chosen step/tick averaged over batches
                    train_accuracies_most_certain_perkamon.append((all_targets == all_predictions_most_certain).reshape(all_targets.shape[0], -1).all(-1).mean()) # Scalar
                    ...
                    # Calculate full kamon accuracy
                    test_accuracies_most_certain_perkamon.append((all_targets == all_predictions_most_certain).reshape(all_targets.shape[0], -1).all(-1).mean()) # Scalar
                    ...
                    # Plot full kamon accuracy
                    axacc_train.plot(iters, train_accuracies_most_certain_perkamon, 'r-', alpha=0.6, label='Full Kamon')
                    axacc_test.plot(iters, test_accuracies_most_certain_perkamon, 'r-', alpha=0.6, label='Full Kamon')
            ...
            if (bi % args.save_every == 0 or bi == args.training_iterations - 1) and bi != start_iter:
                ...
                checkpoint_data = {
                    ...
                    'train_accuracies_most_certain_perkamon': train_accuracies_most_certain_perkamon, # List of scalars
                    'test_accuracies_most_certain_perkamon': test_accuracies_most_certain_perkamon,   # List of scalars
                    'iters': iters,
