#!/usr/bin/env python3
"""Standalone inference script for CLIP-based Kamon image-to-text model."""
...
from clip_image_to_text_model import CLIPImageToTextModel
...
flags.DEFINE_string('output_file', 'inference_results_clip.jsonl', 'Output JSONL file')
...
def load_checkpoint(checkpoint_path, device):
    ...
    also_train_clip = model_config.get('also_train_clip', False)
    ...
    if 'ngram_length' in model_config:
        ...
        # First, we need to determine clip_feature_dim from the model
        # For clip-vit-base-patch32, it's 512
        # Input dim = clip_feature_dim + (ngram_length - 1) * (clip_feature_dim + vocab_size)
        clip_feature_dim = 512  # Default for clip-vit-base-patch32
        feature_combiner_input_dim = checkpoint['model_state_dict']['feature_combiner.0.weight'].shape[1]

        # Solve: feature_combiner_input_dim = clip_feature_dim + (ngram_length - 1) * (clip_feature_dim + vocab_size)
        # Rearrange: (feature_combiner_input_dim - clip_feature_dim) = (ngram_length - 1) * (clip_feature_dim + vocab_size)
        # ngram_length = 1 + (feature_combiner_input_dim - clip_feature_dim) / (clip_feature_dim + vocab_size)
        ngram_length = 1 + (feature_combiner_input_dim - clip_feature_dim) // (clip_feature_dim + vocab_size)
        print(f"Inferred ngram_length = {ngram_length} from checkpoint dimensions")

    # Create model
    model = CLIPImageToTextModel(
        vocab_size=vocab_size,
        max_seq_len=max_seq_len,
        image_size=image_size,
        ngram_length=ngram_length,
        hidden_dim=hidden_dim,
        also_train_clip=also_train_clip,
        use_masks=use_masks,
...
def main(argv):
    ...
    try:
        ...
        if checkpoint_metadata.get('use_lora', False):
            print(f"Using LoRA: r={checkpoint_metadata['lora_r']}, alpha={checkpoint_metadata['lora_alpha']}")
